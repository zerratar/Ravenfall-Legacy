...
public class VendorItem : PacketHandler<TradeItemRequest>
{
	...
	public override async void Handle(TradeItemRequest data, GameClient client)
	{
		...
		var vendorCount = await Game.RavenNest.Players.VendorItemAsync(player.UserId, item.Item.Id, item.Amount);
		if (vendorCount > 0)
		{
...
public class GiftItem : PacketHandler<TradeItemRequest>
{
	...
	public override async void Handle(TradeItemRequest data, GameClient client)
	{
		...
		var giftCount = await Game.RavenNest.Players.GiftItemAsync(player.UserId, item.Player.UserId, item.Item.Id, item.Amount);
		if (giftCount > 0)
		{
