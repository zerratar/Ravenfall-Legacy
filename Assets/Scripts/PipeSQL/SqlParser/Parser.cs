...
namespace SqlParser
{
    ...
    public class MaybeParsed<T>
    {
        public MaybeParsed(bool Parsed, T Result)
        {
            this.Parsed = Parsed;
            this.Result = Result;
        }
    ...
    public class Parser
    {
        ...
        public SqlExpression ParseExpr()
        {
            _depthGuard.Decrease();
        ...
        public SqlExpression ParseSubExpression(short precedence)
        {
        ...
        public SqlExpression ParseIntervalExpr()
        {
            short precedence = 0;
            var expr = ParsePrefix();
        ...
        public SqlExpression ParsePrefix()
        {
            ...
            SqlExpression ParseCaseExpr()
            {
                ...
                var conditions = new Sequence<SqlExpression>();
                var results = new Sequence<SqlExpression>();
            ...
            SqlExpression ParseCastExpression(Func<SqlExpression, DataType, SqlExpression> create)
            {
                return ExpectParens(() =>
                {
                    var expr = ParseExpr();
            ...
            SqlExpression ParseCeilFloorExpr(bool isCeiling)
            {
                return ExpectParens<SqlExpression>(() =>
                {
                    var expr = ParseExpr();
            ...
            SqlExpression.Overlay ParseOverlayExpr()
            {
                ...
                return ExpectParens(() =>
                {
                    ...
                    SqlExpression? forExpr = null;

                    if (ParseKeyword(Keyword.FOR))
                    {
                        forExpr = ParseExpr();
                    }
                    return new SqlExpression.Overlay(expr, whatExpr, fromExpr, forExpr);
                });
            }
            ...
            SqlExpression.Array ParseLeftArray()
            {
                ExpectToken<LeftBracket>();
                return ParseArrayExpr(true);
            }

            SqlExpression ParseLeftParen()
            {
                SqlExpression expr;
                ...
                else
                {
                    ...
                    expr = expressions.Count switch
                    {
                        ...
                        _ => new SqlExpression.Tuple(expressions)
                    };
                }
            ...
            SqlExpression ParseMultipart(Word word)
            {
                var tkn = PeekToken();
            ...
            SqlExpression.Array ParseArrayExpr(bool named)
            {
                if (PeekToken() is RightBracket)
                {
                    NextToken();
                    return new SqlExpression.Array(new ArrayExpression(new Sequence<SqlExpression>(), named));
                }

                var expressions = ParseCommaSeparated(ParseExpr);
                ExpectToken<RightBracket>();
                return new SqlExpression.Array(new ArrayExpression(expressions, named));
            }
        ...
        public SqlExpression ParseFunction(ObjectName name)
        {
            ExpectLeftParen();
            var distinct = ParseAllOrDistinct();
        ...
        /// </summary>
        /// <returns>Expression</returns>
        public SqlExpression ParseGroupByExpr()
        {
            ...
            SqlExpression CreateGroupExpr(bool liftSingleton, bool allowEmpty, Func<Sequence<Sequence<SqlExpression>>, SqlExpression> create)
            {
                return ExpectParens(() =>
                {
        ...
        /// </summary>
        public Sequence<SqlExpression> ParseTuple(bool liftSingleton, bool allowEmpty)
        {
            if (liftSingleton)
            {
                if (ConsumeToken<LeftParen>())
                {
                    Sequence<SqlExpression> result;
                    if (allowEmpty && ConsumeToken<RightParen>())
                    {
                        result = new Sequence<SqlExpression>();
                    }
                    else
                    {
                        var expressions = ParseCommaSeparated(ParseExpr);
                ...
                return new Sequence<SqlExpression> { ParseExpr() };
            }

            ExpectLeftParen();
            Sequence<SqlExpression> exprList;

            if (allowEmpty && ConsumeToken<RightParen>())
            {
                exprList = new Sequence<SqlExpression>();
            }
            else
            {
                var expressions = ParseCommaSeparated(ParseExpr);
        ...
        public SqlExpression ParseInterval()
        {
        ...
        public SqlExpression ParseInfix(SqlExpression expr, short precedence)
        {
        ...
        public SqlExpression ParseArrayIndex(SqlExpression expr)
        {
            var index = ParseExpr();
            ExpectToken<RightBracket>();
            var indexes = new Sequence<SqlExpression> { index };
        ...
        public SqlExpression ParseMapAccess(SqlExpression expr)
        {
            var key = ParseMapKey();

            ConsumeToken<RightBracket>();
            var keyParts = new Sequence<SqlExpression> { key };
            while (ConsumeToken<LeftBracket>())
            {
        ...
        /// <returns></returns>
        public SqlExpression ParseIn(SqlExpression expr, bool negated)
        {
            ...
            var inOp = ExpectParens<SqlExpression>(() =>
            {
        ...
        /// </summary>
        public SqlExpression ParseBetween(SqlExpression expr, bool negated)
        {
        ...
        /// <param name="expr"></param>
        /// <returns></returns>
        public SqlExpression ParsePgCast(SqlExpression expr)
        {
        ...
        public static Value.Number ParseNumeric(Number number)
        {
            ...
            var parsed = double.TryParse(value.Replace('.', ','), NumberStyles.Any, CultureInfo.InvariantCulture, out _);
        ...
        /// </summary>
        /// <returns></returns>
        public SqlExpression ParseMapKey()
        {
            var token = NextToken();
            return token switch
            {
...
#pragma warning restore CS0162
        ...
        public Statement ParseSet()
        {
            ...
            {
                var values = new Sequence<SqlExpression>();
                while (true)
                {
                    try
                    {
        ...
        public TableFactor ParseTableFactor()
        {
            ...
            if (ConsumeToken<LeftParen>())
            {
                ...
                var v = MaybeParseChecked(() => ParseDerivedTableFactor(IsLateral.NotLateral));
                var parsed = v.Parsed;
                var result = v.Result;
        ...
        public SqlExpression? ParseLimit()
        {
        ...
        public Ast.Fetch ParseFetch()
        {
            ...
            var percent = false;
            var quantity = ParseInit<SqlExpression>(ParseOneOfKeywords(Keyword.ROW, Keyword.ROWS) == Keyword.undefined,
                () =>
                {
        ...
        public Values ParseValues(bool allowEmpty)
        {
            ...
            var rows = ParseCommaSeparated(() =>
            {
                ...
                {
                    NextToken();
                    return new Sequence<SqlExpression>();
                }
        ...
        public Sequence<SequenceOptions> ParseCreateSequenceOptions()
        {
            ...
            {
                sequenceOptions.Add(new SequenceOptions.MinValue(new MinMaxValue.Empty()));
            }
            ...
            {
                sequenceOptions.Add(new SequenceOptions.MaxValue(new MinMaxValue.Empty()));
            }
